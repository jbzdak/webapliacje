...
def zad2():
  ...
  step = end / 1e5
  a = 0
  for elem in range(int(1e5)):
      a += math.sin(elem * step)
  return a
...
def zad3_arange(start, stop, step):
  ...
  * step --- krok iteracji
  ...
  """
  a = []
  for elem in range(int((stop - start) // step + 1)):
    a.append(start + elem * step)
  return a

ZAD3_DATA  = [
  {'imie':'Reksio', 'rasa':'pies', 'lapy':True},
  {'imie':'Mruczek', 'rasa':'kot', 'lapy':True},
  {'imie':'Tuptus', 'rasa':'krolik', 'lapy':True},
  {'imie':'Io', 'rasa':'ryba', 'lapy':False},
  {'imie':'Pip', 'rasa':'mysz', 'lapy':True},
]
"""

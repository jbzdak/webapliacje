...
def zad1():
  """
    koemntarz tylko po to by 'for' sie zmiescil w funkcji (helper_getsource) ktorą Pan napisał,
     oczywiście mogłem zmienić również w "if var >= 2:" dwójkę na 0 i też by hulało, ale tamtej funkcji nie ruszam
  """
...
def zad2():
  ...
  for i in range(100001):
      wyn=math.sin(start+i*end/100000)*end/100000
      wynik+=wyn
  return wynik
...
def zad3_arange(start, stop, step):
  ...
  * step --- krok iteracji
  ...
  while True:
    l=(start+i*step)
    l="%.3f" % l #by uciąć tę końcówkę w liczbie (tą spodowoaną translacją liczby
    l=float(l) #na system binarny i z powrotem) wystarczy bodajże jak dam %.12f, dla wygody oczu dałem .3
    if l<stop:
        list.append(l)
        i+=1
    else:
